...
class EncoderElectra(nn.Module):
    def __init__(self, input_dim, emb_dim, hid_dim, model_name):
        super().__init__()
...
class EncoderGRUTFM(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size,
                 cnn_channel=64, gru_layers=2, tfm_layers=2, dropout=0.5):
        super(EncoderGRUTFM, self).__init__()
        ...
        padding_vec = torch.zeros((1, 1, embedding_size))
        self.register_buffer('padding_vec', padding_vec)
        self.conv_3 = nn.Conv2d(1, cnn_channel, (3, embedding_size))
        self.conv_5 = nn.Conv2d(1, cnn_channel, (5, embedding_size))

        self.gru = nn.GRU(embedding_size, hidden_size, gru_layers, dropout=dropout, bidirectional=True)
        encoder_layer = nn.TransformerEncoderLayer(hidden_size, hidden_size // 64, dim_feedforward=hidden_size * 4,
                                                   dropout=dropout)
        self.gru_norm = nn.LayerNorm(hidden_size)

        self.tfm = nn.TransformerEncoder(encoder_layer, tfm_layers)

        pe = torch.zeros(MAX_INPUT_LENGTH, hidden_size)
        ...
        self.trans = nn.Linear(embedding_size + hidden_size * 2, hidden_size)
        self.norm = nn.LayerNorm(hidden_size)

    def cnn_padding(self, x):
        padding_vec = self.padding_vec.repeat(1, x.size(1), 1)
        cnn3_in = torch.cat([padding_vec] * 1 + [x] + [padding_vec] * 1, dim=0).transpose(0, 1).unsqueeze(1)
        cnn5_in = torch.cat([padding_vec] * 2 + [x] + [padding_vec] * 2, dim=0).transpose(0, 1).unsqueeze(1)
        return cnn3_in, cnn5_in
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        rnn_in = embedded
        packed = torch.nn.utils.rnn.pack_padded_sequence(rnn_in, input_lengths)
        outputs, hidden = self.gru(packed, hidden)
        outputs, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(outputs)  # unpack (back to padded)
        rnn_out = outputs[:, :, :self.hidden_size] + outputs[:, :, self.hidden_size:]  # Sum bidirectional outputs
        rnn_out = self.gru_norm(rnn_out)
        # S x B x H
        ...
        tfm_in = rnn_out
        tfm_out = self.tfm(tfm_in, src_key_padding_mask=seq_mask)

        outs = self.norm(self.trans(torch.cat([embedded, rnn_out, tfm_out], dim=-1)))

        return outs, tfm_out[0]
