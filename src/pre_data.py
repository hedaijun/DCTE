...
class Lang:
    ...
    def build_input_lang(self, trim_min_count):  # build the input lang vocab and dict
        if trim_min_count > 0:
            ...
            self.index2word = ["PAD", "NUM", "UNK", "SOS"] + self.index2word
        else:
            self.index2word = ["PAD", "NUM", "SOS"] + self.index2word
        self.word2index = {}
...
def load_ape_data(filename):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
    f = open(filename, encoding="utf-8")
    data = []
    for s in f:
...
def transfer_ape_num(data, id_file):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        input_seq = ["NUM" if w == '\t' else w for w in re.sub(pattern, '\t', text)]
...
def transfer_english_num(data):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        for s in seg:
            ...
            if pos:
                if pos.start() > 0:
                    input_seq.append(s[:pos.start()].lower())
                ...
                if pos.end() < len(s):
                    input_seq.append(s[pos.end():].lower())
            else:
                input_seq.append(s.lower())
...
def prepare_data_mapws(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, file, tree=False,
                       isTrans=False):
    input_lang = Lang(file)
    output_lang = Lang()
    train_pairs = []
    ...
    for pair in pairs_trained:
        ...
        for word in pair[1]:  # ���ʽ
            ...
            if word not in output_lang.index2word:
                flag_not = False
                for i, j in enumerate(pair[2]):
                    if abs(float(j) - float(word)) < 1e-4:
                        temp_num.append(i)  # ������������������δ֪���е�λ��
