...
class Beam:  # the class save the beam node
    def __init__(self, score, input_var, hidden, all_output, attn=None):
        self.score = score
...
class TreeBeam:  # the class save the beam node
    def __init__(self, score, node_stack, embedding_stack, left_childs, out, attn=None):
        self.score = score
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            for tv, ti in zip(topv.split(1, dim=1), topi.split(1, dim=1)):
                ...
                out_token = int(ti)
                if out_token >= output_lang.n_words:
                    print(out_token)
                ...
                attn = copy.copy(b.attn)
                attn.append(current_attn)
                current_beams.append(TreeBeam(b.score + float(tv), current_node_stack, current_embeddings_stacks,
                                              current_left_childs, current_out, attn))
        ...
        if flag:
            break
    attn = torch.cat(beams[0].attn, dim=0).squeeze(1)
    if USE_CUDA:
        attn = attn.cpu()
    return beams[0].out, attn.detach().numpy()
